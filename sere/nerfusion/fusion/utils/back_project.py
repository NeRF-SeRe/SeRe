...
def back_project(coords, origin, voxel_size, feats, KRcam):
    ...
    dim: (num of voxels,)
    '''
    # TODO: h and w are too small here, maybe the scale of KRcam is wrong
    ...
    for batch in range(bs):
        ...
        # visualize_backprojection(coords, count)
    return feature_volume_all, count


import trimesh
def visualize_backprojection(coords, count, size=0.1):
    '''
    :param coords: coordinates of voxels,
    dim: (num of voxels, 4) (4 : batch ind, x, y, z)
    :param count: number of times each voxel can be seen
    dim: (num of voxels,)
    :param size: size of the voxel visualized
    '''
    ...
    for coord, cnt in zip(coords, count):
        sphere = trimesh.creation.icosphere(radius=size, color=[cnt, cnt, cnt])
        transform = trimesh.transformations.translation_matrix(coord)
