# ported from NeRFusion (https://github.com/jetd1/NeRFusion)
import torch
...
def sparse_to_dense_channel(locs, values, dim, c, default_val, device):
    dense = torch.full([dim[0], dim[1], dim[2], c], float(default_val), device=device, dtype=values.dtype)
    if locs.shape[0] > 0:
        dense[locs[:, 0], locs[:, 1], locs[:, 2]] = values
    return dense
...
class GRUFusion(nn.Module):
    ...
    def __init__(self, opt, ch_in=None, voxel_size=None, direct_substitute=False):
        ...
        # self.n_scales = len(opt.THRESHOLDS) - 1
        self.n_scales = opt.n_fusion_levels
        self.voxel_size = voxel_size if voxel_size is not None else opt.bound * 2 / opt.resolution
    ...
    def convert2dense(self, current_coords, current_values, coords_target_global, tsdf_target, relative_origin,
        ...
        # dim = (torch.Tensor(self.opt.N_VOX).cuda() // 2 ** (self.opt.N_LAYER - scale - 1)).int()
        # dim = (torch.Tensor([2048 * self.opt.bound] * 3).cuda() // 2 ** (self.opt.n_fusion_levels - scale - 1)).int()
        dim = (torch.Tensor([self.opt.resolution] * 3).cuda() // 2 ** (self.opt.n_fusion_levels - scale - 1)).int()
        ...
        if self.opt.fusion_full is False:
        ...
        if self.opt.fusion_full is True:
    ...
    def save_mesh(self, scale, outputs, scene):
        ...
        # outputs['origin'].append(min_c * self.opt.VOXEL_SIZE * (2 ** (self.opt.N_LAYER - scale - 1)))
        outputs['origin'].append(min_c * self.opt.bound / 2 ** scale)
    ...
    def forward(self, coords, values_in, inputs, scale=2, outputs=None, save_mesh=False):
        ...
        interval = 2 ** (self.opt.n_fusion_levels - scale - 1)
        ...
        for i in range(batch_size):
            ...
            # voxel_size = self.opt.VOXEL_SIZE * interval
            voxel_size = self.opt.bound / 2 ** scale
            ...
            #     # get partial gt
            #     occ_target = inputs['occ_list'][self.opt.N_LAYER - scale - 1][i]
            #     tsdf_target = inputs['tsdf_list'][self.opt.N_LAYER - scale - 1][i][occ_target]
            ...
            if not self.direct_substitude:
                ...
                r_coords = inputs['world_to_aligned_camera'][:3, :] @ r_coords
